...
pub enum BleSersorType {
    Xiaomi,
    LinkyTIC,
}

impl Display for BleSersorType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            BleSersorType::Xiaomi => write!(f, "Xiaomi"),
            BleSersorType::LinkyTIC => write!(f, "Linky TIC"),
        }
    }
}

#[derive(Debug, Clone)]
pub struct BleSensorStats {
    pub rx_packets: u64,
}

#[derive(Debug, Clone)]
pub struct BleSensor {
    pub sensor_type: BleSersorType,
    pub ble_addr: BleAddress,
    pub name: String,
    pub last_seen: DateTime<Utc>,
    pub measurements: BleMeasurement, // Last measurement received
    pub stats: BleSensorStats,
...
impl BleSensor {
    pub fn init(
        ...
        Self {
            ...
            name,
            stats: BleSensorStats { rx_packets: 1 }, // At least one packet received
    ...
    pub fn default_name(sensor_type: &BleSersorType, ble_addr: &BleAddress) -> String {
        format!("{} {}", sensor_type, ble_addr.mac_manufacturer_part())
    }
    ...
    pub fn get_alert(&self) -> Option<SensorAlert> {
        if self.is_battery_critical().unwrap_or(false) {
            ...
            Some(SensorAlert::new_notification(
                format!(
...
impl FilterableSensor for BleSensor {
